...
class Jdf
{
    ...
    public static function convertTimestampToJalali($carbonTimeStamp, $format = 'Y/n/j H:i:s')
    {
        if (is_null($carbonTimeStamp)){
            return '';
        }
        return self::formatDate(
            Carbon::createFromFormat('Y-m-d H:i:s', $carbonTimeStamp),
            $format,
            false
        );
    }

    public static function formatDate(Carbon $date, $format = 'Y/n/j - H:i', $fa = true)
    {
        return self::jdate($format, $date->timestamp, '', 'Asia/Tehran', $fa ? "fa" : "en");
    }
}
